...
You can also access colors in JavaScript through the `tailwind` utility exported from `@jasonmorganson/tailwind-config`, like so:

```tsx title="component.tsx"
import { tailwind } from '@jasonmorganson/tailwind-config';
