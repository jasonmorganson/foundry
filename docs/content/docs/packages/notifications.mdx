...
To render an in-app notifications feed, import the `NotificationsTrigger` component from the `@jasonmorganson/notifications` package and use it in your app. We've already added this to the sidebar in the example app:
...
```tsx title="notify.ts"
import { notifications } from '@jasonmorganson/notifications';
