...
Our `@jasonmorganson/seo` package provides a JSON+LD helper built on `schema-dts`, allowing for structured data generation in a type-safe way. You can declare your own JSON+LD implementations like so:

```tsx title="page.tsx"
import { JsonLd } from '@jasonmorganson/seo/json-ld';
import type { WithContext, YourInterface } from '@jasonmorganson/seo/json-ld';
