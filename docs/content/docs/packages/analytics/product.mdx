...
To capture product events, you can use the `analytics` object exported from the `@jasonmorganson/analytics` package. Since analytics is optional, use optional chaining when calling methods.
...
import { analytics } from '@jasonmorganson/analytics';
```
