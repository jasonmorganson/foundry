...
We use [Resend](https://resend.com/) to send transactional emails. The templates, located in `@jasonmorganson/email`, are powered by [React Email](https://react.email/) - a collection of high-quality, unstyled components for creating beautiful emails using React and TypeScript.
...
To send an email, you can use the `resend` object, which is imported from the `@jasonmorganson/email` package:
