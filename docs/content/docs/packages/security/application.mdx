...
Both the `web` and `app` apps have Arcjet configured with a central client at `@jasonmorganson/security`  that includes the Shield WAF rules. Each app then extends this client with additional rules:
