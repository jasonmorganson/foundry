...
To send a webhook, simply use the `send` function from the `@jasonmorganson/webhooks` package:
...
To get access to the application portal, use the `getAppPortal` function from `@jasonmorganson/webhooks` and use the returned URL in an `iframe` on your dashboard.
