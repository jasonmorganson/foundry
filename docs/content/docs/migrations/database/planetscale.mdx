...
## 3. Swap out the required dependencies in `@jasonmorganson/database`

Uninstall the existing dependencies...

```package-install
npm uninstall @neondatabase/serverless @prisma/adapter-neon ws @types/ws --filter @jasonmorganson/database
```

...and install the new ones:

```package-install
npm install @planetscale/database @prisma/adapter-planetscale --filter @jasonmorganson/database
```
