...
## 1. Swap out the required dependencies in `@jasonmorganson/database`

Uninstall the existing dependencies...

```package-install
npm uninstall @prisma/adapter-neon @prisma/client prisma --filter @jasonmorganson/database
```

...and install the new ones:

```package-install
npm install drizzle-orm @neondatabase/serverless --filter @jasonmorganson/database
npm install -D drizzle-kit --filter @jasonmorganson/database
```

## 2. Update the database connection code

Delete everything in `@jasonmorganson/database/index.ts` and replace it with the following:
...
import { env } from '@jasonmorganson/env';
...
import { page } from '@jasonmorganson/database/schema';
