...
```package-install
npm uninstall @neondatabase/serverless @prisma/adapter-neon @prisma/client prisma ws @types/ws --filter @jasonmorganson/database
```

...and install the Appwrite dependency:

```package-install
npm install node-appwrite --filter @jasonmorganson/database
```
...
import { database, databaseId, ID, Permission, Role } from '@jasonmorganson/database';
...
```ts
import { database, databaseId, ID } from '@jasonmorganson/database';
...
```ts
import { database, databaseId, Query } from '@jasonmorganson/database';
...
```ts
import { database, databaseId } from '@jasonmorganson/database';
...
```ts
import { client } from '@jasonmorganson/auth/client';
