...
## 2. Swap out the required dependencies in `@jasonmorganson/database`
...
## 3. Setup EdgeDB in `@jasonmorganson/database` package

In the `@jasonmorganson/database` directory, run:
...
You can also delete `prisma/` directory from the `@jasonmorganson/database`:
...
import edgeql from '@jasonmorganson/database/dbschema/edgeql-js';
