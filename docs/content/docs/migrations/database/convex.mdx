...
```package-install
npm install convex --filter @jasonmorganson/database
```
...
import { ConvexClientProvider } from '@jasonmorganson/database/provider';
...
Delete the `prisma/` directory from `@jasonmorganson/database`:
...
import { api } from '@jasonmorganson/convex/_generated/api';
