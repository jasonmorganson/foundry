...
```package-install
npm install appwrite node-appwrite --filter @jasonmorganson/auth
```
...
import { createAdminClient } from '@jasonmorganson/auth/server';
import { cookies } from 'next/headers';
...
// After (Appwrite)
'use client';
import { account } from '@jasonmorganson/auth/client';
