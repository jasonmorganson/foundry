...
```package-install
npm uninstall @clerk/nextjs @clerk/themes @clerk/types --filter @jasonmorganson/auth
```

...and install the Supabase Auth dependencies:

```package-install
npm install @supabase/supabase-js @supabase/ssr --filter @jasonmorganson/auth
```

Additionally, add `@jasonmorganson/database` to the `auth` package dependencies to enable organization/team management.
...
import { createClient } from '@jasonmorganson/auth/server';
...
// After (Supabase)
'use client';
import { createClient } from '@jasonmorganson/auth/client';
