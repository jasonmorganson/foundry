...
```package-install
npm install better-auth next --filter @jasonmorganson/auth
```

Additionally, add `@jasonmorganson/database` to the `auth` package dependencies.
...
export { POST, GET } from '@jasonmorganson/auth/handlers'
```
