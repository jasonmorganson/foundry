...
```package-install
npm uninstall @jasonmorganson/analytics --filter @jasonmorganson/feature-flags
```

Then, install the new dependencies:

```package-install
npm install hypertune server-only --filter @jasonmorganson/feature-flags
```
...
```sh title="Terminal"
bun run build --filter @jasonmorganson/feature-flags
```
